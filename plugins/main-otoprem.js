...
let handler = async (m, { text, command, usedPrefix }) => {
	...
	if (!harganya) return m.reply('Jumlahnya berapa?');
	if (harganya < 1000) return m.reply('Minimal 1.000 ya.');
	if (harganya > 1000000) return m.reply('Emg bneran?');
	...
	while (status !== 'completed') {
		...
		if (res && res.status === 'completed') {
			...
			m.reply(`Premium sudah aktif, Silahkan ketik /limit untuk melihat status premium`)
